...
# Ensure /etc/resolv.conf is fresh and populated with host Android DNS & public fallbacks
DNS_CONF="$ROOTFS_DIR/etc/resolv.conf"
if [ -L "$DNS_CONF" ]; then
    rm -f "$DNS_CONF" 2>/dev/null || true
fi
{
    echo "# Generated by MobileLinux Host Launcher"
    echo "options timeout:2 attempts:2 rotate ndots:1"
    for d in $(getprop 2>/dev/null | grep -E '\[net\.(dns|eth|wlan)[0-9]*\]' | awk -F': ' '{print $2}' | tr -d '[]\r\n '); do
        if [ -n "$d" ] && ! echo "$d" | grep -q ':'; then
            echo "nameserver $d"
        fi
    done
...
} > "$DNS_CONF" 2>/dev/null || true
chmod 644 "$DNS_CONF" 2>/dev/null || true
