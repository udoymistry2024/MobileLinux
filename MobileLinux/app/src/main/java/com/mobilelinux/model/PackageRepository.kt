    ...
    val desktopPackages: List<LinuxPackage> by lazy {
        listOf(
            ...
            LinuxPackage(
                ...
                installCommand = "echo '[MobileLinux] [ 10%] Configuring official Mozilla APT repository...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo rm -f /etc/apt/sources.list.d/mozilla*.list /etc/apt/keyrings/packages.mozilla.org* /etc/apt/preferences.d/mozilla 2>/dev/null || true; if [ -f /usr/bin/firefox ] && [ ! -x /usr/lib/firefox/firefox ]; then sudo apt-get purge -y firefox 2>/dev/null || true; fi; sudo apt-get update && sudo apt-get install -y --no-install-recommends ca-certificates wget gpg gnupg 2>&1 && sudo install -d -m 0755 /etc/apt/keyrings && wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O- | sudo tee /etc/apt/keyrings/packages.mozilla.org.asc >/dev/null && echo 'deb [signed-by=/etc/apt/keyrings/packages.mozilla.org.asc] https://packages.mozilla.org/apt mozilla main' | sudo tee /etc/apt/sources.list.d/mozilla.list >/dev/null && printf 'Package: *\\nPin: origin packages.mozilla.org\\nPin-Priority: 1000\\n\\nPackage: firefox*\\nPin: release o=Ubuntu\\nPin-Priority: -1\\n' | sudo tee /etc/apt/preferences.d/mozilla >/dev/null && echo '[MobileLinux] [ 50%] Updating Mozilla package repository...' && sudo apt-get update 2>&1 && echo '[MobileLinux] [ 75%] Installing official native Firefox browser...' && sudo apt-get install -y firefox 2>&1 && (sudo update-alternatives --set x-www-browser /usr/bin/firefox 2>/dev/null || true) && (test -x /usr/lib/firefox/firefox || test -x /usr/bin/firefox) && echo '[MobileLinux] [100%] Firefox installed successfully!'",
                checkInstalledCommand = "(test -x /usr/lib/firefox/firefox || test -x /usr/bin/firefox-esr) || ([ -x /usr/bin/firefox ] && ! grep -q 'snap' /usr/bin/firefox 2>/dev/null)",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging native Firefox...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y firefox 2>&1 || true; sudo rm -f /etc/apt/preferences.d/mozilla /etc/apt/sources.list.d/mozilla*.list /etc/apt/keyrings/packages.mozilla.org* /usr/local/bin/firefox /usr/share/applications/firefox.desktop 2>/dev/null || true; rm -rf /home/ubuntu/.mozilla /root/.mozilla /home/ubuntu/.cache/mozilla /root/.cache/mozilla 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] Firefox uninstalled cleanly!'"
            ),
            LinuxPackage(
                id = "webapp-manager",
