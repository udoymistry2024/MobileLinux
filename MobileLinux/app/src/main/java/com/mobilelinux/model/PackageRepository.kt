    ...
    val desktopPackages: List<LinuxPackage> by lazy {
        listOf(
            ...
            LinuxPackage(
                ...
                installCommand = "echo '[MobileLinux] [ 20%] Configuring official Mozilla APT repository...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get update && sudo apt-get install -y --no-install-recommends ca-certificates wget gpg 2>&1; sudo install -d -m 0755 /etc/apt/keyrings 2>/dev/null || true; wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O- | sudo tee /etc/apt/keyrings/packages.mozilla.org.gpg >/dev/null; echo 'deb [signed-by=/etc/apt/keyrings/packages.mozilla.org.gpg] https://packages.mozilla.org/apt mozilla main' | sudo tee /etc/apt/sources.list.d/mozilla.list >/dev/null; printf 'Package: firefox*\\nPin: origin packages.mozilla.org\\nPin-Priority: 1001\\n' | sudo tee /etc/apt/preferences.d/mozilla >/dev/null; echo '[MobileLinux] [ 60%] Updating package lists...'; sudo apt-get update 2>&1; echo '[MobileLinux] [ 80%] Installing Firefox GUI browser...'; sudo apt-get install -y firefox 2>&1 && (sudo update-alternatives --set x-www-browser /usr/bin/firefox 2>/dev/null || true) && echo '[MobileLinux] [100%] Firefox installed successfully!'",
                checkInstalledCommand = "test -x /usr/lib/firefox/firefox || test -x /usr/bin/firefox-esr",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging native Firefox...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y firefox 2>&1 || true; sudo rm -f /etc/apt/preferences.d/mozilla /etc/apt/sources.list.d/mozilla.list /etc/apt/keyrings/packages.mozilla.org.gpg /usr/local/bin/firefox /usr/share/applications/firefox.desktop 2>/dev/null || true; rm -rf /home/ubuntu/.mozilla /root/.mozilla /home/ubuntu/.cache/mozilla /root/.cache/mozilla 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] Firefox uninstalled and cleaned!'"
            ),
            LinuxPackage(
                id = "webapp-manager",
                ...
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging WebApp Manager...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y webapp-manager 2>&1 || true; rm -f /tmp/webapp-manager.deb 2>/dev/null || true; rm -rf /home/ubuntu/.local/share/ice /root/.local/share/ice /home/ubuntu/.config/webapp-manager /root/.config/webapp-manager 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] WebApp Manager uninstalled and cleaned!'"
            ),
            LinuxPackage(
