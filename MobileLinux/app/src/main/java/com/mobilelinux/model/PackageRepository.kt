...
object PackageRepository {
    ...
    fun getUninstallCommand(pkg: LinuxPackage): String {
        ...
        when (pkg.id) {
            ...
            "xfce4-desktop" -> {
                return "echo '[MobileLinux] [ 20%] Stopping active desktop sessions...'; " +
                        "desktop-stop 2>/dev/null || true; " +
                        ...
                        "echo '[MobileLinux] [ 40%] Purging XFCE4 and TigerVNC packages...'; " +
                        "export DEBIAN_FRONTEND=noninteractive; " +
                        "sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; " +
                        "sudo apt-get -o DPkg::Lock::Timeout=60 purge -y xfce4 xfce4-terminal tigervnc-standalone-server tigervnc-common dbus-x11 2>&1 || true; " +
                        "echo '[MobileLinux] [ 70%] Removing orphaned dependencies...'; " +
                        "sudo apt-get -o DPkg::Lock::Timeout=60 autoremove -y --purge 2>&1 || true; " +
                        "sudo apt-get clean 2>/dev/null || true; " +
                        "echo '[MobileLinux] [ 90%] Cleaning desktop caches and configurations...'; " +
                        "rm -rf /home/ubuntu/.vnc /root/.vnc /home/ubuntu/.config/xfce4 /root/.config/xfce4 /tmp/.X11-unix /tmp/.X*-lock 2>/dev/null || true; " +
                        "sudo rm -f /usr/bin/startxfce4 /usr/bin/vncserver /usr/bin/tigervncserver /usr/bin/xfce4-session 2>/dev/null || true; " +
                        "echo '[MobileLinux] [100%] ✓ Desktop Mode uninstalled cleanly!'"
            }
            "gemini-cli" -> {
    ...
    val desktopPackages: List<LinuxPackage> by lazy {
        listOf(
            LinuxPackage(
                ...
                installCommand = "export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get update && sudo apt-get install -y --no-install-recommends xfce4 xfce4-terminal tigervnc-standalone-server tigervnc-common dbus-x11 2>&1 && (command -v startxfce4 || test -x /usr/bin/startxfce4) && echo '[MobileLinux] [100%] Desktop Mode installed successfully!'",
            ...
            LinuxPackage(
                ...
                category = PackageCategory.DESKTOP_APPS,
                version = "Latest (Native DEB / Portable)",
                ...
                installCommand = "echo '[MobileLinux] [ 10%] Preparing native Firefox installation...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo rm -f /etc/apt/sources.list.d/mozilla*.list /etc/apt/keyrings/packages.mozilla.org* /etc/apt/preferences.d/mozilla 2>/dev/null || true; if [ -f /usr/bin/firefox ] && [ ! -x /usr/lib/firefox/firefox ] && [ ! -x /opt/firefox/firefox ]; then sudo apt-get purge -y firefox 2>/dev/null || true; fi; sudo apt-get -o DPkg::Lock::Timeout=30 update 2>&1 && sudo apt-get -o DPkg::Lock::Timeout=30 install -y --no-install-recommends ca-certificates wget curl gpg gnupg 2>&1 && sudo install -d -m 0755 /etc/apt/keyrings /etc/apt/preferences.d 2>/dev/null || true; echo '[MobileLinux] [ 25%] Configuring Mozilla Team PPA (ARM64 & x86_64)...' && (wget -q \"https://keyserver.ubuntu.com/pks/lookup?op=get&search=0x0ab215679c571d1c8325275b9bdb3d89ce49ec21\" -O- 2>/dev/null | sudo gpg --dearmor --yes -o /etc/apt/keyrings/mozillateam.gpg 2>/dev/null || curl -fsSL \"https://keyserver.ubuntu.com/pks/lookup?op=get&search=0x0ab215679c571d1c8325275b9bdb3d89ce49ec21\" 2>/dev/null | sudo gpg --dearmor --yes -o /etc/apt/keyrings/mozillateam.gpg 2>/dev/null || true) && echo 'deb [signed-by=/etc/apt/keyrings/mozillateam.gpg] https://ppa.launchpadcontent.net/mozillateam/ppa/ubuntu noble main' | sudo tee /etc/apt/sources.list.d/mozillateam.list >/dev/null && printf 'Package: *\\nPin: release o=LP-PPA-mozillateam\\nPin-Priority: 1001\\n\\nPackage: firefox*\\nPin: release o=Ubuntu\\nPin-Priority: -1\\n' | sudo tee /etc/apt/preferences.d/mozillateam-ppa >/dev/null && echo '[MobileLinux] [ 50%] Updating package lists for Mozilla Team PPA...' && (sudo apt-get -o DPkg::Lock::Timeout=60 update 2>&1 || true) && echo '[MobileLinux] [ 70%] Installing native Firefox browser...' && (sudo apt-get -o DPkg::Lock::Timeout=60 install -y --no-install-recommends firefox 2>&1 || sudo apt-get -o DPkg::Lock::Timeout=60 install -y --no-install-recommends firefox-esr 2>&1 || true); if [ -x /usr/lib/firefox/firefox ] || [ -x /usr/bin/firefox-esr ] || ([ -x /usr/bin/firefox ] && ! grep -q 'snap' /usr/bin/firefox 2>/dev/null); then echo '[MobileLinux] [ 90%] Configuring desktop launcher and browser alternatives...'; sudo update-alternatives --set x-www-browser /usr/bin/firefox 2>/dev/null || true; else echo '[MobileLinux] [ 75%] Fetching official Mozilla Linux portable bundle...'; ARCH=\$(uname -m); if [ \"\$ARCH\" = \"aarch64\" ] || [ \"\$ARCH\" = \"arm64\" ]; then BUNDLE_URL=\"https://download.mozilla.org/?product=firefox-latest&os=linux64-aarch64&lang=en-US\"; else BUNDLE_URL=\"https://download.mozilla.org/?product=firefox-latest&os=linux64&lang=en-US\"; fi; (wget -q --show-progress \"\$BUNDLE_URL\" -O /tmp/firefox.tar.bz2 2>&1 || curl -L \"\$BUNDLE_URL\" -o /tmp/firefox.tar.bz2 2>&1) && sudo mkdir -p /opt && sudo tar -xjf /tmp/firefox.tar.bz2 -C /opt/ 2>&1 && sudo ln -sf /opt/firefox/firefox /usr/bin/firefox && rm -f /tmp/firefox.tar.bz2; fi; (test -x /usr/lib/firefox/firefox || test -x /opt/firefox/firefox || test -x /usr/bin/firefox-esr || test -x /usr/bin/firefox) && echo '[MobileLinux] [100%] Firefox installed successfully!'",
                checkInstalledCommand = "test -x /usr/lib/firefox/firefox || test -x /opt/firefox/firefox || test -x /usr/bin/firefox-esr || ([ -x /usr/bin/firefox ] && ! grep -q 'snap' /usr/bin/firefox 2>/dev/null)",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging native Firefox...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y firefox firefox-esr 2>&1 || true; sudo rm -rf /opt/firefox /usr/local/bin/firefox /etc/apt/preferences.d/mozillateam* /etc/apt/preferences.d/mozilla* /etc/apt/sources.list.d/mozillateam*.list /etc/apt/sources.list.d/mozilla*.list /etc/apt/keyrings/mozillateam* /etc/apt/keyrings/packages.mozilla.org* /usr/share/applications/firefox.desktop 2>/dev/null || true; rm -rf /home/ubuntu/.mozilla /root/.mozilla /home/ubuntu/.cache/mozilla /root/.cache/mozilla 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] Firefox uninstalled cleanly!'"
            ),
            LinuxPackage(
                id = "vscode-gui",
                name = "Visual Studio Code (GUI)",
                category = PackageCategory.DESKTOP_APPS,
                version = "Latest (Official DEB)",
                description = "Microsoft's full graphical code editor with extension marketplace, IntelliSense, Git integration, and multi-language debugging for the desktop environment.",
                installCommand = "echo '[MobileLinux] [ 10%] Preparing Microsoft Visual Studio Code repository...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get -o DPkg::Lock::Timeout=30 update 2>&1 && sudo apt-get -o DPkg::Lock::Timeout=30 install -y --no-install-recommends wget curl gpg ca-certificates apt-transport-https 2>&1 && sudo install -d -m 0755 /etc/apt/keyrings 2>/dev/null || true; echo '[MobileLinux] [ 25%] Importing Microsoft GPG signing key...'; (wget -qO- https://packages.microsoft.com/keys/microsoft.asc 2>/dev/null | sudo gpg --dearmor --yes -o /etc/apt/keyrings/packages.microsoft.gpg 2>/dev/null || curl -fsSL https://packages.microsoft.com/keys/microsoft.asc 2>/dev/null | sudo gpg --dearmor --yes -o /etc/apt/keyrings/packages.microsoft.gpg 2>/dev/null || true); echo 'deb [arch=amd64,arm64,armhf signed-by=/etc/apt/keyrings/packages.microsoft.gpg] https://packages.microsoft.com/repos/code stable main' | sudo tee /etc/apt/sources.list.d/vscode.list >/dev/null; echo '[MobileLinux] [ 40%] Updating package index for VS Code...'; (sudo apt-get -o DPkg::Lock::Timeout=60 update 2>&1 || true); echo '[MobileLinux] [ 60%] Installing Visual Studio Code (GUI)...'; (sudo apt-get -o DPkg::Lock::Timeout=120 install -y --no-install-recommends code 2>&1 || true); if [ ! -f /usr/share/code/code ] && [ ! -x /usr/bin/code ]; then echo '[MobileLinux] [ 70%] Fetching direct Microsoft VS Code DEB package...'; ARCH=\$(uname -m); if [ \"\$ARCH\" = \"aarch64\" ] || [ \"\$ARCH\" = \"arm64\" ]; then DEB_URL=\"https://update.code.visualstudio.com/latest/linux-deb-arm64/stable\"; elif [ \"\$ARCH\" = \"x86_64\" ] || [ \"\$ARCH\" = \"amd64\" ]; then DEB_URL=\"https://update.code.visualstudio.com/latest/linux-deb-x64/stable\"; else DEB_URL=\"https://update.code.visualstudio.com/latest/linux-deb-armhf/stable\"; fi; (wget -q --show-progress \"\$DEB_URL\" -O /tmp/code.deb 2>&1 || curl -L \"\$DEB_URL\" -o /tmp/code.deb 2>&1) && (sudo dpkg -i /tmp/code.deb 2>&1 || sudo apt-get install -f -y 2>&1) && rm -f /tmp/code.deb; fi; echo '[MobileLinux] [ 85%] Configuring PRoot compatibility wrapper & desktop integration...'; sudo bash -c 'cat << \"CODE_EOF\" > /usr/local/bin/code\n#!/bin/bash\nexport DISPLAY=\"\${DISPLAY:-:1}\"\nexport LIBGL_ALWAYS_SOFTWARE=1\nexport NO_AT_BRIDGE=1\nexport DONT_PROMPT_WSL_INSTALL=1\n\nREAL_CODE=\"/usr/share/code/code\"\n[ ! -f \"\$REAL_CODE\" ] && REAL_CODE=\"/usr/bin/code\"\n[ ! -f \"\$REAL_CODE\" ] && REAL_CODE=\"\$(command -v code 2>/dev/null)\"\n\nEXTRA_FLAGS=(\"--no-sandbox\" \"--disable-gpu\" \"--disable-dev-shm-usage\")\n\nif [ \"\$(id -u)\" = \"0\" ]; then\n    mkdir -p /root/.config/Code /root/.vscode 2>/dev/null || true\n    EXTRA_FLAGS+=(\"--user-data-dir=/root/.config/Code\")\n    exec \"\$REAL_CODE\" \"\${EXTRA_FLAGS[@]}\" \"\$@\"\nelse\n    mkdir -p \"\$HOME/.config/Code\" \"\$HOME/.vscode\" 2>/dev/null || true\n    exec \"\$REAL_CODE\" \"\${EXTRA_FLAGS[@]}\" \"\$@\"\nfi\nCODE_EOF'; sudo chmod +x /usr/local/bin/code 2>/dev/null || true; if [ -f /usr/share/applications/code.desktop ]; then sudo sed -i 's|^Exec=/usr/share/code/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code.desktop 2>/dev/null || true; sudo sed -i 's|^Exec=/usr/bin/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code.desktop 2>/dev/null || true; fi; if [ -f /usr/share/applications/code-url-handler.desktop ]; then sudo sed -i 's|^Exec=/usr/share/code/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code-url-handler.desktop 2>/dev/null || true; sudo sed -i 's|^Exec=/usr/bin/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code-url-handler.desktop 2>/dev/null || true; fi; (test -x /usr/share/code/code || test -x /usr/bin/code || test -x /usr/local/bin/code) && echo '[MobileLinux] [100%] Visual Studio Code installed successfully!'",
                checkInstalledCommand = "test -x /usr/share/code/code || test -x /usr/bin/code || test -x /usr/local/bin/code",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging Visual Studio Code...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y code 2>&1 || true; sudo rm -f /usr/local/bin/code /etc/apt/sources.list.d/vscode.list /etc/apt/keyrings/packages.microsoft.gpg /usr/share/applications/code*.desktop /tmp/code.deb 2>/dev/null || true; rm -rf /home/ubuntu/.config/Code /root/.config/Code /home/ubuntu/.vscode /root/.vscode 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] Visual Studio Code uninstalled cleanly!'"
            ),
            LinuxPackage(
                id = "webapp-manager",
                ...
                installCommand = "echo '[MobileLinux] [ 20%] Preparing dependencies for WebApp Manager...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get update && sudo apt-get install -y --no-install-recommends wget python3 python3-gi gir1.2-gtk-3.0 gir1.2-xapp-1.0 python3-bs4 python3-configobj python3-pil python3-setproctitle python3-tldextract xapps-common 2>&1 && echo '[MobileLinux] [ 50%] Downloading WebApp Manager v1.4.6...' && (wget -q \"https://mirrors.edge.kernel.org/linuxmint-packages/pool/main/w/webapp-manager/webapp-manager_1.4.6_all.deb\" -O /tmp/webapp-manager.deb || wget -q \"http://packages.linuxmint.com/pool/main/w/webapp-manager/webapp-manager_1.4.6_all.deb\" -O /tmp/webapp-manager.deb) && echo '[MobileLinux] [ 75%] Installing WebApp Manager deb package...' && (sudo dpkg -i /tmp/webapp-manager.deb 2>&1 || sudo apt-get install -f -y 2>&1) && rm -f /tmp/webapp-manager.deb && test -x /usr/bin/webapp-manager && echo '[MobileLinux] [100%] WebApp Manager installed successfully!'",
                checkInstalledCommand = "test -x /usr/bin/webapp-manager",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging WebApp Manager...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y webapp-manager 2>&1 || true; rm -f /tmp/webapp-manager.deb 2>/dev/null || true; rm -rf /home/ubuntu/.local/share/ice /root/.local/share/ice /home/ubuntu/.config/webapp-manager /root/.config/webapp-manager 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] WebApp Manager uninstalled cleanly!'"
            ),
            LinuxPackage(
                ...
                installCommand = "echo '[MobileLinux] [ 20%] Installing LibreOffice Suite (Writer, Calc, Impress)...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get update && sudo apt-get install -y libreoffice-writer libreoffice-calc libreoffice-impress libreoffice-common libreoffice-gtk3 libreoffice-style-colibre fonts-dejavu-core 2>&1 && sudo mkdir -p /prod 2>/dev/null || true; sudo touch /prod/version 2>/dev/null || true; if [ -f /usr/lib/libreoffice/program/soffice.bin ]; then (if [ -f /usr/lib/libreoffice/program/oosplash ] && [ ! -f /usr/lib/libreoffice/program/oosplash.orig ]; then sudo mv /usr/lib/libreoffice/program/oosplash /usr/lib/libreoffice/program/oosplash.orig 2>/dev/null || true; fi; sudo bash -c 'cat << \"LO_EOF\" > /usr/lib/libreoffice/program/oosplash\n#!/bin/bash\nDIR=\"\$(dirname \"\$0\")\"\nmkdir -p /prod 2>/dev/null || true\n[ ! -f /prod/version ] && touch /prod/version 2>/dev/null || true\nrm -f /tmp/OSL_PIPE_* /tmp/.osl_* 2>/dev/null || true\nif [ -z \"\$SAL_USE_VCLPLUGIN\" ]; then\n    if [ -f \"\$DIR/libvclplug_gtk3lo.so\" ]; then\n        export SAL_USE_VCLPLUGIN=gtk3\n    else\n        export SAL_USE_VCLPLUGIN=gen\n    fi\nfi\nexec \"\$DIR/soffice.bin\" \"\$@\"\nLO_EOF'; sudo chmod +x /usr/lib/libreoffice/program/oosplash 2>/dev/null || true); fi; (test -x /usr/bin/libreoffice || test -x /usr/bin/soffice || test -x /usr/bin/lowriter) && echo '[MobileLinux] [100%] LibreOffice installed successfully!'",
                checkInstalledCommand = "test -x /usr/bin/libreoffice || test -x /usr/bin/soffice || test -x /usr/bin/lowriter",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging LibreOffice Suite...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y \"libreoffice*\" 2>&1 || true; sudo rm -f /usr/local/bin/libreoffice /usr/local/bin/soffice /usr/local/bin/lowriter /usr/local/bin/localc /usr/local/bin/loimpress 2>/dev/null || true; rm -rf /home/ubuntu/.config/libreoffice /root/.config/libreoffice /etc/libreoffice /usr/lib/libreoffice 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] LibreOffice uninstalled cleanly!'"
            ),
            LinuxPackage(
                id = "gimp-gui",
                ...
                installCommand = "echo '[MobileLinux] [ 20%] Installing GIMP Image Editor...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get update && sudo apt-get install -y --no-install-recommends gimp 2>&1 && test -x /usr/bin/gimp && echo '[MobileLinux] [100%] GIMP installed successfully!'",
                checkInstalledCommand = "test -x /usr/bin/gimp",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging GIMP Image Editor...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y gimp gimp-data 2>&1 || true; rm -rf /home/ubuntu/.config/GIMP /root/.config/GIMP /home/ubuntu/.cache/gimp /root/.cache/gimp 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] GIMP uninstalled cleanly!'"
            ),
            LinuxPackage(
                id = "evince-gui",
                ...
                installCommand = "echo '[MobileLinux] [ 20%] Installing Evince Document & PDF Viewer...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get update && sudo apt-get install -y evince 2>&1 && test -x /usr/bin/evince && echo '[MobileLinux] [100%] Evince Document Viewer installed successfully!'",
                checkInstalledCommand = "test -x /usr/bin/evince",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging Evince Document Viewer...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y evince evince-common 2>&1 || true; rm -rf /home/ubuntu/.config/evince /root/.config/evince /home/ubuntu/.cache/evince /root/.cache/evince 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] Evince uninstalled cleanly!'"
            ),
            LinuxPackage(
                id = "vlc-gui",
                ...
                installCommand = "echo '[MobileLinux] [ 20%] Installing VLC Media Player & GUI dependencies...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get update && sudo apt-get install --reinstall -y vlc vlc-bin vlc-plugin-qt vlc-plugin-skins2 vlc-plugin-base libxcb-cursor0 libxcb-xinerama0 libqt5svg5 libqt5x11extras5 2>&1 && (if [ -f /usr/bin/vlc ] && head -c 4 /usr/bin/vlc 2>/dev/null | grep -q $'\\x7FELF'; then sudo cp -f /usr/bin/vlc /usr/bin/vlc.real 2>/dev/null || true; fi; if [ -f /usr/bin/vlc.real ]; then sudo sed -i 's/geteuid/getppid/g' /usr/bin/vlc.real 2>/dev/null || true; fi; for lib in /usr/lib/*-linux-gnu*/libvlccore*; do if [ -f \"\$lib\" ]; then sudo sed -i 's/geteuid/getppid/g' \"\$lib\" 2>/dev/null || true; fi; done; for p in /usr/lib/*-linux-gnu*/vlc; do if [ -x \"\$p/vlc-cache-gen\" ]; then sudo \"\$p/vlc-cache-gen\" \"\$p/plugins\" 2>/dev/null || true; fi; done; if [ -f /usr/share/applications/vlc.desktop ]; then sudo sed -i 's/^Exec=.*vlc.*$/Exec=\\/usr\\/bin\\/vlc %U/g' /usr/share/applications/vlc.desktop 2>/dev/null || true; fi; sudo bash -c 'cat << \"VLC_EOF\" > /usr/bin/vlc\n#!/bin/bash\nexport DISPLAY=\"\${DISPLAY:-:1}\"\nexport QT_X11_NO_MITSHM=1\nexport QT_QPA_PLATFORM=xcb\nexport LIBGL_ALWAYS_SOFTWARE=1\nexport XDG_RUNTIME_DIR=\"\${XDG_RUNTIME_DIR:-/tmp}\"\nexport NO_AT_BRIDGE=1\nunset GDK_SYNCHRONIZE\n\nREAL_VLC=\"/usr/bin/vlc.real\"\nif [ ! -f \"\$REAL_VLC\" ] || ! head -c 4 \"\$REAL_VLC\" 2>/dev/null | grep -q $\"\\x7FELF\"; then\n    REAL_VLC=\"/usr/bin/vlc\"\nfi\n\nVLC_ARGS=()\nhas_intf=0\nis_info=0\nfor arg in \"\$@\"; do\n    case \"\$arg\" in\n        -I|--intf|--extraintf) has_intf=1 ;;\n        -h|--help|--version|-v) is_info=1 ;;\n    esac\ndone\nif [ \$is_info -eq 0 ]; then\n    if [ \$has_intf -eq 0 ]; then\n        VLC_ARGS+=(\"-I\" \"qt\")\n    fi\n    VLC_ARGS+=(\"--no-dbus\" \"--avcodec-hw=none\" \"--vout=xcb_x11\")\nfi\n\nchmod 777 /tmp/.X11-unix /tmp/.X11-unix/* 2>/dev/null || true\nxhost + >/dev/null 2>&1 || true\n\nif [ \"\$(id -u)\" = \"0\" ]; then\n    exec su - ubuntu -c \"export DISPLAY='\$DISPLAY'; export QT_X11_NO_MITSHM=1; export QT_QPA_PLATFORM=xcb; export XDG_RUNTIME_DIR=/tmp; export LIBGL_ALWAYS_SOFTWARE=1; export NO_AT_BRIDGE=1; unset GDK_SYNCHRONIZE; '\$REAL_VLC' \${VLC_ARGS[*]} \$(printf '%q ' \\\"\\$@\\\")\"\nelse\n    exec \"\$REAL_VLC\" \"\${VLC_ARGS[@]}\" \"\$@\"\nfi\nVLC_EOF'; sudo chmod +x /usr/bin/vlc 2>/dev/null || true; sudo cp -f /usr/bin/vlc /usr/local/bin/vlc 2>/dev/null || true) && (test -x /usr/bin/vlc || test -x /usr/bin/vlc.real) && echo '[MobileLinux] [100%] VLC Media Player installed successfully!'",
                checkInstalledCommand = "test -x /usr/bin/vlc || test -x /usr/bin/vlc.real",
                uninstallCommand = "echo '[MobileLinux] [ 20%] Purging VLC Media Player...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y \"vlc*\" 2>&1 || true; sudo rm -f /usr/local/bin/vlc /usr/bin/vlc /usr/bin/vlc.real /usr/bin/vlc-bin 2>/dev/null || true; rm -rf /home/ubuntu/.config/vlc /root/.config/vlc /home/ubuntu/.cache/vlc /root/.cache/vlc 2>/dev/null || true; echo '[MobileLinux] [ 60%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] VLC Media Player uninstalled cleanly!'"
            )
        )
    }
