    ...
    val desktopPackages: List<LinuxPackage> by lazy {
        listOf(
            ...
            LinuxPackage(
                ...
                checkInstalledCommand = "test -f /usr/lib/firefox/firefox || test -f /usr/bin/firefox-esr || (test -f /usr/bin/firefox && [ ! -L /usr/bin/firefox ] && grep -vq 'snap' /usr/bin/firefox 2>/dev/null)",
                uninstallCommand = "echo '[MobileLinux] [ 30%] Purging Firefox...'; export DEBIAN_FRONTEND=noninteractive; sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true; sudo apt-get purge -y firefox 2>&1 || true; sudo rm -f /etc/apt/preferences.d/mozilla /etc/apt/sources.list.d/mozilla.list /etc/apt/keyrings/packages.mozilla.org.gpg 2>/dev/null || true; echo '[MobileLinux] [ 70%] Removing unused dependencies...'; sudo apt-get autoremove -y --purge 2>&1 || true; sudo apt-get clean 2>/dev/null || true; echo '[MobileLinux] [100%] Firefox uninstalled cleanly!'"
            ),
            LinuxPackage(
                id = "webapp-manager",
            ...
            LinuxPackage(
                ...
                checkInstalledCommand = "command -v evince || test -x /usr/bin/evince",
            ...
            LinuxPackage(
                ...
                checkInstalledCommand = "command -v vlc && test -f /usr/bin/vlc",
