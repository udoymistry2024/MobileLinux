...
class UbuntuRuntime(private val context: Context) {
            ...
            try {
                ...
                try {
                    ...
                    if (sourcesListD.exists()) {
                        File(sourcesListD, "ubuntu.sources").delete()
                    }
    ...
    /**
     * Dynamically synchronizes host Android network DNS servers and fallback public nameservers
     * into Ubuntu rootfs /etc/resolv.conf and /etc/hosts.
     * Prevents "Temporary failure in name resolution" when connected to Wi-Fi routers or mobile data.
     */
    fun syncDnsConfiguration() {
        try {
            val etcDir = File(rootfsDir, "etc")
            ...
            // 1. Extract active DNS from Android ConnectivityManager (Current Wi-Fi / Mobile DNS)
            try {
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    ...
                    linkProperties?.dnsServers?.forEach { inetAddr ->
                        ...
                        // Prioritize IPv4 DNS addresses
                        if (!host.isNullOrBlank() && !host.contains(":") && !host.startsWith("127.")) {
                            dnsList.add(host)
                        }
                    }
                }
            } catch (e: Exception) {
                Log.w(TAG, "DNS from ConnectivityManager notice: ${e.message}")
            }

            // 2. Extract DNS from Android getprop (net.dns1, net.dns2, net.wlan.dns, etc.)
            try {
                ...
                while (reader.readLine().also { line = it } != null) {
                    val l = line ?: break
                    if (l.contains("[net.dns") || l.contains("[net.eth") || l.contains("[net.wlan")) {
                        val match = Regex("\\[(net\\.[^\\]]+)\\]:\\s*\\[([^\\]]+)\\]").find(l)
                        if (match != null) {
                            val value = match.groupValues[2].trim()
                            if (value.isNotEmpty() && !value.contains(":") && !value.startsWith("127.")) {
                                dnsList.add(value)
                            }
                        }
                    }
            ...
            // 3. Ultra-reliable global public fallback nameservers
            dnsList.add("1.1.1.1")         // Cloudflare
            dnsList.add("8.8.8.8")         // Google
            dnsList.add("1.0.0.1")         // Cloudflare secondary
            dnsList.add("8.8.4.4")         // Google secondary
            dnsList.add("9.9.9.9")         // Quad9
            dnsList.add("208.67.222.222")  // OpenDNS

            // 4. Safely write /etc/resolv.conf
            val resolvConf = File(etcDir, "resolv.conf")
            try {
            ...
            sb.append("# Generated by MobileLinux DNS Manager\n")
            sb.append("options timeout:2 attempts:2 rotate ndots:1\n")
            for (dns in dnsList) {
                sb.append("nameserver $dns\n")
            }
            safeWriteFile(resolvConf, sb.toString())
            resolvConf.setReadable(true, false)

            // 5. Safely ensure /etc/hosts has local loopbacks
            val hostsFile = File(etcDir, "hosts")
            val existingHosts = if (hostsFile.exists()) hostsFile.readText() else ""
            if (!existingHosts.contains("mobilelinux")) {
                val hostsContent = "127.0.0.1   localhost\n127.0.1.1   mobilelinux\n::1         localhost ip6-localhost ip6-loopback\n"
                safeWriteFile(hostsFile, hostsContent)
                hostsFile.setReadable(true, false)
            }
            Log.d(TAG, "DNS configuration synchronized with ${dnsList.size} nameservers")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to sync DNS: ${e.message}", e)
        }
    }
    ...
    fun createSessionProcess(
        ...
        // Synchronize active DNS before running command or terminal session
        syncDnsConfiguration()
    ...
    fun installCommandWrappers() {
        ...
        try {
            ...
            try {
                ...
                // Restore official Ubuntu ports APT sources in sources.list & clean obsolete files
                try {
                    val aptDir = File(rootfsDir, "etc/apt")
                    if (aptDir.exists()) {
                        val sourcesListD = File(aptDir, "sources.list.d")
                        val ubuntuSources = File(sourcesListD, "ubuntu.sources")
                        if (ubuntuSources.exists()) {
                            ubuntuSources.delete()
                        }
                        ...
                        val officialSources = """
                            ...
                            deb http://ports.ubuntu.com/ubuntu-ports/ noble-security main restricted universe multiverse
                        """.trimIndent() + "\n"
                        safeWriteFile(sourcesList, officialSources)
                        sourcesList.setReadable(true, false)
                    }
                } catch (ignored: Exception) {}

                // Ensure offline get-pip.py is available in rootfs
                try {
                    val shareDir = File(rootfsDir, "usr/local/share/mobilelinux")
                    ensureRealDirectory(shareDir)
                    val getPipTarget = File(shareDir, "get-pip.py")
                    if (!getPipTarget.exists() || getPipTarget.length() == 0L) {
                        context.assets.open("scripts/get-pip.py").use { input ->
                            getPipTarget.outputStream().use { output ->
                                input.copyTo(output)
                            }
                        }
                        getPipTarget.setReadable(true, false)
                    }
                } catch (e: Exception) {
                    Log.w(TAG, "Notice provisioning get-pip.py: ${e.message}")
                }
            } catch (ignored: Exception) {}

            // Ensure ensure-pip CLI wrapper is installed and executable
            val ensurePipFile = File(usrLocalBin, "ensure-pip")
            safeWriteFile(ensurePipFile, getEnsurePipScript())
            ensurePipFile.setExecutable(true, false)
            ensurePipFile.setReadable(true, false)
            ...
            // install-tools & pkg-install & pkg-uninstall utilities
            ...
            val pkgUninstallFile = File(usrLocalBin, "pkg-uninstall")
            safeWriteFile(pkgUninstallFile, getPkgUninstallScript())
            pkgUninstallFile.setExecutable(true, false)
            pkgUninstallFile.setReadable(true, false)
            ...
            if (vlcBin.exists() || vlcReal.exists()) {
                ...
                val vlcDesktop = File(rootfsDir, "usr/share/applications/vlc.desktop")
                if (vlcDesktop.exists()) {
                    try {
                        val content = vlcDesktop.readText()
                        val updated = content.replace(Regex("(?m)^Exec=.*vlc.*$"), "Exec=/usr/bin/vlc %U")
                        if (content != updated) {
                            safeWriteFile(vlcDesktop, updated)
                        }
                    } catch (ignored: Exception) {}
                }
            } else {
            ...
            try {
                ...
                for (cBin in targetBins) {
                    val staleEnsurePip = File(cBin, "ensure-pip")
                    if (staleEnsurePip.exists()) staleEnsurePip.delete()
        ...
        try {
            ...
            try {
                ...
                if (condaBinDir.exists() && condaBinDir.isDirectory) {
                    val toolsToLink = listOf(
                        "pkg-install", "pkg-uninstall", "ensure-pip",
                        "pkg-install-python", "pkg-uninstall-python", "conda-sync-packages", "conda-sync",
                        "conda-manager", "install-jupyter", "jupyter", "jupyter-start", "jupyter-restart", "jupyter-notebook", "jupyter-lab",
                        "fix-jupyter-mobile", "xdg-open", "x-www-browser", "sensible-browser", "www-browser",
                        "desktop-start", "start-desktop", "desktop-stop", "stop-desktop"
    ...
    fun startDesktopProcess(resolution: String): Process {
        stopDesktopProcess()
        ensureBashConfigured()
        installCommandWrappers()
        ...
        val proc = createSessionProcess(
            sessionId = "desktop_${System.currentTimeMillis()}",
            cols = 120,
            rows = 40,
            execCommand = "/usr/local/bin/desktop-start $resolution"
        )
        desktopProcess = proc
        return proc
    }

    fun stopDesktopProcess() {
        try {
    ...
    fun isDesktopProcessRunning(): Boolean {
        return desktopProcess?.isAlive == true
    }
    ...
    private fun getDesktopStartScript(): String = listOf(
        ...
        "fi",
        "if [ -f /usr/bin/vlc.real ] || dpkg -s vlc >/dev/null 2>&1 || dpkg -s vlc-bin >/dev/null 2>&1; then",
        ...
        "",
        "if [ \"\$(id -u)\" = \"0\" ]; then",
        "    exec su - ubuntu -s /bin/bash -c 'export DISPLAY=\"\$DISPLAY\"; export QT_X11_NO_MITSHM=1; export QT_QPA_PLATFORM=xcb; export XDG_RUNTIME_DIR=/tmp; export LIBGL_ALWAYS_SOFTWARE=1; export NO_AT_BRIDGE=1; unset GDK_SYNCHRONIZE; REAL_VLC=\"\$1\"; shift; exec \"\$REAL_VLC\" \"\$@\"' -- \"\$REAL_VLC\" \"\${VLC_ARGS[@]}\" \"\$@\"",
        "else",
        "    exec \"\$REAL_VLC\" \"\${VLC_ARGS[@]}\" \"\$@\"",
        "fi",
        "VLC_EOF",
        ...
        "fi",
        "",
        "# Visual Studio Code sandbox & root bypass patch",
        "if [ -f /usr/share/code/code ] || [ -f /usr/bin/code ] || dpkg -s code >/dev/null 2>&1; then",
        "    if [ -f /usr/share/applications/code.desktop ]; then",
        "        sed -i 's|^Exec=/usr/share/code/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code.desktop 2>/dev/null || true",
        "        sed -i 's|^Exec=/usr/bin/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code.desktop 2>/dev/null || true",
        "    fi",
        "    if [ -f /usr/share/applications/code-url-handler.desktop ]; then",
        "        sed -i 's|^Exec=/usr/share/code/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code-url-handler.desktop 2>/dev/null || true",
        "        sed -i 's|^Exec=/usr/bin/code.*|Exec=/usr/local/bin/code %F|g' /usr/share/applications/code-url-handler.desktop 2>/dev/null || true",
        "    fi",
        "    cat << 'CODE_EOF' > /usr/local/bin/code",
        "#!/bin/bash",
        "export DISPLAY=\"\${DISPLAY:-:1}\"",
        "export LIBGL_ALWAYS_SOFTWARE=1",
        "export NO_AT_BRIDGE=1",
        ...
        "            break",
        "        fi",
        "        echo \"[MobileLinux] \$xbin direct launch did not stay alive. Falling back...\"",
        "        VNC_PID=\"\"",
        "    fi",
        "done",
        "",
        ...
        "done",
        "",
        "# 10. Launch XFCE desktop in the foreground to keep the PRoot container alive",
        "if [ \$STARTED -eq 1 ]; then",
        "    echo -e \"\\033[1;32m[MobileLinux]\\033[0m Desktop display :1 ready on port 5901.\"",
        "    [ -r \"\$HOME/.Xresources\" ] && xrdb \"\$HOME/.Xresources\" 2>/dev/null",
        "    if command -v startxfce4 >/dev/null 2>&1; then",
        "        echo \"[MobileLinux] Executing XFCE4 desktop session...\"",
        "        exec dbus-launch --exit-with-session startxfce4",
        "    elif command -v xfce4-session >/dev/null 2>&1; then",
        "        echo \"[MobileLinux] Executing xfce4-session...\"",
        "        exec dbus-launch --exit-with-session xfce4-session",
        "    elif [ -n \"\$VNC_PID\" ]; then",
        "        echo \"[MobileLinux] Waiting on display server process...\"",
        "        wait \$VNC_PID",
        "    fi",
        "else",
    ...
    private fun getUbuntuBashrc(): String = listOf(
        ...
        "alias pkg-uninstall='/usr/local/bin/pkg-uninstall'",
        "alias ensure-pip='/usr/local/bin/ensure-pip'",
    ...
    private fun getEnsurePipScript(): String = listOf(
        "#!/bin/bash",
        "# MobileLinux Ensure Pip Tool",
        "export PATH=\"/home/ubuntu/.local/bin:/root/.local/bin:/home/ubuntu/miniforge3/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:\$PATH\"",
        "if /usr/bin/python3 -m pip --version >/dev/null 2>&1 || [ -x /home/ubuntu/.local/bin/pip ] || [ -x /usr/bin/pip3 ]; then",
        "    echo \"[MobileLinux] pip is already installed!\"",
        "    exit 0",
        "fi",
        "echo -e \"\\033[1;36m[MobileLinux]\\033[0m Setting up pip...\"",
        ...
        "# Try apt-get first",
        "if sudo apt-get -o DPkg::Lock::Timeout=60 -o Acquire::ForceIPv4=true install -y --no-install-recommends python3-pip python3-setuptools python3-wheel 2>/dev/null; then",
        "    echo -e \"\\033[1;32m[MobileLinux]\\033[0m Successfully installed python3-pip via apt!\"",
        "    exit 0",
        "fi",
        "# Fallback: get-pip.py",
        "GET_PIP=\"/usr/local/share/mobilelinux/get-pip.py\"",
        "if [ -f \"\$GET_PIP\" ]; then",
        "    echo -e \"\\033[1;33m[MobileLinux]\\033[0m Running offline get-pip.py bootstrap...\"",
        "    sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* 2>/dev/null || true",
        "    sudo apt-get -o DPkg::Lock::Timeout=30 install -y --no-install-recommends python3-distutils 2>/dev/null || true",
        "    if python3 \"\$GET_PIP\" --break-system-packages --no-warn-script-location; then",
        "        echo -e \"\\033[1;32m[MobileLinux]\\033[0m Successfully installed pip via get-pip.py!\"",
        "        exit 0",
        "    fi",
        "fi",
    ...
    private fun getPipWrapperScript(): String = listOf(
        ...
        "    export DEBIAN_FRONTEND=noninteractive",
        "    sudo rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock* /var/lib/dpkg/updates/* 2>/dev/null || true",
        "    (sudo apt-get -o DPkg::Lock::Timeout=60 -o Acquire::ForceIPv4=true install -y --no-install-recommends python3-pip || ((sudo apt-get -o DPkg::Lock::Timeout=60 -o Acquire::ForceIPv4=true update || true) && sudo apt-get -o DPkg::Lock::Timeout=60 -o Acquire::ForceIPv4=true install -y --no-install-recommends python3-pip)) || true",
        "fi",
        "if [ -n \"\$CONDA_PREFIX\" ] && [ -x \"\$CONDA_PREFIX/bin/pip\" ]; then",
        "    exec \"\$CONDA_PREFIX/bin/pip\" \"\$@\"",
        "elif [ -x /home/ubuntu/miniforge3/bin/pip ]; then",
    ...
    private fun getSmartToolWrapperScript(cmdName: String, pkgName: String): String = listOf(
        ...
        "    if /usr/local/bin/pkg-install \"$pkgName\"; then",
    ...
    private fun getInstallToolsScript(): String = listOf(
        ...
        "if [ -x /usr/local/bin/pkg-install ]; then",
        "    if /usr/local/bin/pkg-install nano vim-tiny git python3-pip htop tree unzip zip; then",
    ...
    private fun getPkgUninstallScript(): String = listOf(
        "#!/bin/bash",
        "# MobileLinux Universal Smart Package Uninstaller",
        "export DEBIAN_FRONTEND=noninteractive",
        "if [ \$# -eq 0 ]; then",
        "    echo \"Usage: pkg-uninstall <package_name> [packages...]\"",
        "    exit 1",
        "fi",
    ...
    private fun getPkgInstallPythonScript(): String = listOf(
        ...
        "MODULE_NAME=\"\$3\"",
        "if [ -z \"\$PIP_PKG\" ]; then",
        "    echo \"Usage: pkg-install-python <pip_package_name> [apt_package_name] [module_name]\"",
        "    exit 1",
        "fi",
        "# Auto-resolve Python import module name if not explicitly provided",
        "if [ -z \"\$MODULE_NAME\" ]; then",
        "    case \"\$PIP_PKG\" in",
        "        opencv-python|opencv-contrib-python|opencv-python-headless) MODULE_NAME=\"cv2\" ;;",
        "        scikit-learn) MODULE_NAME=\"sklearn\" ;;",
        "        scikit-image) MODULE_NAME=\"skimage\" ;;",
        "        Pillow|pillow) MODULE_NAME=\"PIL\" ;;",
        "        beautifulsoup4) MODULE_NAME=\"bs4\" ;;",
        "        PyYAML|pyyaml) MODULE_NAME=\"yaml\" ;;",
        "        sherlock-project) MODULE_NAME=\"sherlock\" ;;",
        "        *) MODULE_NAME=\"\${PIP_PKG//-/_}\" ;;",
        "    esac",
        "fi",
        ...
        "    if [ -x /usr/local/bin/pkg-install ]; then",
        "        if /usr/local/bin/pkg-install \"\$APT_PKG\"; then",
        "            INSTALLED_ANY=1",
        ...
        "        echo -e \"\\033[1;33m[MobileLinux]\\033[0m pip3 not found, installing python3-pip...\"",
        "        if [ -x /usr/local/bin/pkg-install ]; then",
        "            /usr/local/bin/pkg-install python3-pip python3-setuptools python3-wheel || true",
        "        else",
        "            sudo apt-get install -y --no-install-recommends python3-pip python3-setuptools python3-wheel || true",
        "        fi",
        "    fi",
        "    if command -v pip3 >/dev/null 2>&1; then",
        "        if pip3 install --break-system-packages --prefer-binary --no-cache-dir \"\$PIP_PKG\" 2>&1; then",
        "            INSTALLED_ANY=1",
        ...
        "    if [ -d \"\$conda_base\" ]; then",
        "        SEEN_DIRS=\"\$SEEN_DIRS\$conda_base \"",
        "        if \"\$conda_base/bin/python\" -c \"import \$MODULE_NAME\" >/dev/null 2>&1; then",
        ...
        "            if \"\$conda_base/bin/pip\" install --prefer-binary --no-cache-dir --default-timeout=120 --retries 3 \"\$PIP_PKG\" 2>&1; then",
        "                INSTALLED_ANY=1",
        "            fi",
        "        fi",
        "    fi",
        ...
        "        if [[ \"\$SEEN_DIRS\" != *\" \$ENV_DIR \"* ]]; then",
        "            SEEN_DIRS=\"\$SEEN_DIRS\$ENV_DIR \"",
        "            if \"\$ENV_DIR/bin/python\" -c \"import \$MODULE_NAME\" >/dev/null 2>&1; then",
        "                continue",
        "            fi",
        "            \"\$env_pip\" install --prefer-binary --no-cache-dir --default-timeout=120 --retries 3 \"\$PIP_PKG\" 2>&1 || true",
        "        fi",
        "    fi",
        "done",
        "# Active CONDA_PREFIX if set",
        "if [ -n \"\$CONDA_PREFIX\" ] && [ -x \"\$CONDA_PREFIX/bin/pip\" ]; then",
        "    if ! \"\$CONDA_PREFIX/bin/python\" -c \"import \$MODULE_NAME\" >/dev/null 2>&1; then",
        "        \"\$CONDA_PREFIX/bin/pip\" install --prefer-binary --no-cache-dir --default-timeout=120 --retries 3 \"\$PIP_PKG\" 2>&1 || true",
        "    fi",
        "fi",
        ...
        "elif [ -x /root/miniconda3/bin/python ] && /root/miniconda3/bin/python -c \"import \$MODULE_NAME\" >/dev/null 2>&1; then",
        "    VERIFIED=1",
        "elif [ -n \"\$CONDA_PREFIX\" ] && [ -x \"\$CONDA_PREFIX/bin/python\" ] && \"\$CONDA_PREFIX/bin/python\" -c \"import \$MODULE_NAME\" >/dev/null 2>&1; then",
        "    VERIFIED=1",
        "fi",
